...
// This example tests very slow rise time power cycles with StableLCD.
// It has testet more than 300.000 cycles, with no errors reported.
...
void loop() {                                   // Test loop
  ...
  if (settleDelayMs) {                          // If additional power up delay
    uint32_t start = millis();
    while ((millis()-start)<settleDelayMs) beginOk = lcd.initClear();   // Run lcd.initClear() while power up delay
  }
